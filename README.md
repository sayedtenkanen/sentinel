...
| `sentinel/parsers/` | `base.py`, `python.py`, `javascript.py`, `go.py`, `rust.py`, `null.py`, `models.py` | Pluggable parser abstraction — `BaseParser` ABC (14 methods), `PythonParser` (full ast.*), `JavaScriptParser` (regex/line heuristics), `GoParser` (regex/line heuristics), `RustParser` (regex/line heuristics), `NullParser` (safe empty defaults), 11 typed dataclass models |
